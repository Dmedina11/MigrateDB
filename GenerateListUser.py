...
def CreateFileOuputs():
	...
	list_usuarios_not_File = open("FilesOutput/List_usuarios_not_File.txt", 'w')#archivo con la lista de usuarios no existences en csv
	list_usuarios_not_File.close()
	
	logs = open("FilesOutput/GenerateListUser_Logs_error.txt", 'w')#archivo log
	logs.close()
...
def ReadCSV (name_file):
	...
	logs = open("FilesOutput/GenerateListUser_Logs_error.txt", 'a')#archivo log
	...
	try:	
		...
		logs.write("No es posible abrir el archivo que contiene las macs")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: GenerateListUser_Logs_error.txt")
	logs.close()
...
def GetMacsInDB(cursor_db):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetMacsInDB)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: GenerateListUser_Logs_error.txt")
	logs.close()
...
def GetInfoForGenerateCSVOutput(list_macs_notInFile, cursor_db):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetInfoForGenerateCSVOutput)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: GenerateListUser_Logs_error.txt")
	logs.close()
...
def GenerateCSV(info_dicctionary):
	...
	csv = open("FilesOutput/List_usuarios_not_File.txt", 'a')#abrimos archivo de escritura
