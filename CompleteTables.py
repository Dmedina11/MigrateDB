...
def ReadCSV (name_file):
	...
	logs = open("FilesOutput/CompleteTables_Logs_error.txt", 'a')#abrimos archivo log
	...
	try:	
		...
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: CompleteTables_Logs_error.txt")
...
def CreateFileOuputs():
	...
	logs = open("FilesOutput/CompleteTables_Logs_error.txt", 'w')##archivo para el aviso de errores
	logs.close()
...
def CheckExistenceInDB(cursor_db, table, field, data):
	...
	logs = open("FilesOutput/CompleteTables_Logs_error.txt", 'a')#abrimos archivos log
	...
	try:#manejo try/exceot
		...
		logs.write("No es posible hacer una consulta a la base de datos (CheckExistenceInDB)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: CompleteTables_Logs_error.txt")
...
def InsertDataInDB(connection, cursor_db, table, data, sequence):
	...
	try:#manejo de exceptions
		...
		logs.write("No es posible realizar la insercion en la base de datos (InsertDataInDB)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: CompleteTables_Logs_error.txt")
	logs.close()
...
def GetElementesToInsert(cursor_db, table, index):
	...
	logs = open("FilesOutput/CompleteTables_Logs_error.txt", 'a')
	...
	try:
		...
		logs.write("No es posible hacer la consulta a la base de datos (GetElementesToInsert)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: CompleteTables_Logs_error.txt")
...
def main ():#funcion principal
	...
	CreateFileOuputs()#se crean los archivos de salida
	logs = open("FilesOutput/CompleteTables_Logs_error.txt", 'a')#se abre archivo log

	print "this script has created the next output files CompleteTables_Logs_error.txt"

	information_DB = ReadCSV(sys.argv[1])#obtenemos la informacion de la base de datos...

	print "Completando datos en tablas no relacionadas"
