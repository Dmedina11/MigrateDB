...
def GetAllInformationofDataBase(cursor_db, tipo_usuario):
	...
	logs = open("FilesOutput/TablesRelations_Logs_error.txt", 'a')#archivo de log
	...
	#except:#manejo de exception
	#	logs.write("No es posible realizar la consulta a la base de datos (GetAllInformationofDataBase)")
	#	sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetIDEstadoUsuario(cursor_db, id_estadodispositivo):
	...
	try:#manejo de errores	
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetIDEstadoUsuario)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def CheckExistenceInDBUser(cursor_db, table, field, data, field2, data2, field3, data3):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos (CheckExistenceInDB)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def InsertElementInUsuario(cursor_db, connection, sequence, fecha_ini, fecha_fin, rut, tipo_usuario, tipo_estado_usuario):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la insercion de datos en la tabla usuario (InsertElementInUsuario)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def InsertUsuarios(cursor_db_export, connection_db_export, information_generic, kind_user_import, kind_user_export, state_device_import, state_device_export, cursor_db_import):
	...
	print "Insert elements into table usuario"
...
def InsertUsuariosFaltantes(cursor_db_export, connection_db_export, cursor_db_import, kind_user_import, kind_user_export, id_tipo_us):
	...
	try:#manejo de errores
		...
		for rut in ruts_faltantes:
			...
			query = "select usuario.id_tipo_us, usuario.fecha_ini, usuario.fecha_fin from usuario"
	 		cursor_db_import.execute(query)
...
def GetAllRutsUsersAdd(cursor_db_export, connection_db_export):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetAllRutsUsersAdd)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
