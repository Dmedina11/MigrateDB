...
def ReadText (name_file):
	...
	try:
		...
		logs.write("No es posible leer el archivo con la informacion de las macs")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def RemoveMacsNotInCSV(cursor_db, conexion_db, list_macs):
	...
	for mac_db in list_macs_db:
		...
		for mac in list_macs:
			elementos = mac.split(';')
			if elementos[0].upper() == mac_db:#comparamos, si son iguales, existe, rompemos ciclo...
...
def GetIDs(cursor_db, table, field):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetIDs)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def DeleteElement(cursor_db, connection_db, id_element, table, field, option):
	...
	try:
		...
		logs.write("No es posible realizar la eliminacion de datos en la base de datos (DeleteElement)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def UpdateNameEquipo(cursor_db_export, connection_db_export, name_equipo, id_equipo):
	...
	try:
		...
		logs.write("No es posible realizar la actualizacion de nombre al equipo")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()		
...
def GetIdEquipo(cursor_db, mac):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()		
...
def UpdateAllEquipos(cursor_db, conexion_db, list_macs_csv):
	...
	name_equipo = open("FilesOutput/TablesRelations_names.txt", 'w')
	list_macs_db = GetIDs(cursor_db, "mac", "macs")#obtenemos todas las macs de la base de datos...
...
def GetListEquiposConflictivos(cursor_db):

	logs = open("FilesOutput/TablesRelations_Logs_error.txt", 'a')#se abre el archivo log
	list_equipos_conflictivos = [] #guardamos el id de estos equipos...
	
	try:
		query = "select equipo.id, COUNT(*) as cantidad_macs from equipo join macs on (equipo.id = macs.iddispositivo) group by equipo.id"
		cursor_db.execute(query)#ejecucion de la consulta
		for row in cursor_db:
			if row[1] >2:#si la cantidad de macs supera las dos macs se agrega a la lista...
				list_equipos_conflictivos.append(row[0])
	except:
		logs.write("No es posible realizar la consulta a la base de datos (GetListEquiposConflictivos)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetInfoByID(cursor_db, id_equipo):
	...
	try:
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetInfoByID)\n")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetSerialByMac(cursor_db_import, mac):
	...
	try:	
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetSerialByMac)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GenerateCSV(info_dicctionary, cursor_db_import):
	...
	try:	
		...
		logs.write("No es posible generar el csv con la informacion recolectada de los equipos conflictivos")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def DeleteEquiposConflictivos(cursor_db, connection, list_equipos_conflictivos):
	...
	try:
		...
		logs.write("No es posible eliminar los equipos conflictivos (DeleteEquiposConflictivos)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetEquipoID(cursor_db, connexion_db):
	...
	try:
		...
		logs.write("No es posible hacer la consulta a la base de datos (GetEquipoID)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetMacOfEquipo(cursor_db, conexion, id_equipo):
	...
	try:
		...
		logs.write("No es posible hacer la consulta a la base de datos (GetMacOfEquipo)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def main ():
	...
	list_equipos_conflictivos = GetListEquiposConflictivos(data_connected_export[1])#obtenemos la lista de elementos conflictivos...
