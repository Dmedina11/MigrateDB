...
def ConnectDB (host, dbname, user, password):
	...
	logs = open("FilesOutput/GetMacslogs_error.txt", 'a')#abrimos archivo para log en caso de fallar la conexion...
	connection = "host= %s dbname= %s user=%s password=%s" % (host, dbname, user, password)#conexion con la informacion dada
	var = "Connecting to database\n	->%s" % (connection)#impresion de los datos de la conexion
	logs.write(var+"\n")
	...
	try: 
		...
		logs.write("error, no es posible conectarse a la base de datos\n")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: GetMacslogs_error.txt")
	logs.close()
...
def CreateFileOuputs():
	...
	logs = open("FilesOutput/GetMacslogs_error.txt", 'w')#archivo para el aviso de errores
	logs.close()
...
def ReadCSV (name_file):
	...
	logs = open("FilesOutput/GetMacslogs_error.txt", 'a')#abrimos archivo log
...
def CheckMacInDB(mac, cursor_db, table, field):
	...
	try:#con un contador determinamos si existe el elemento seleccionado o no...
		...
		logs.write("No es posible realizar la consulta a la base de datos (CheckMacInDB)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: GetMacslogs_error.txt")
...
def main ():
	...
	CreateFileOuputs()	
	logs = open("FilesOutput/GetMacslogs_error.txt", 'a')
	data_conected = ConnectDB(sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5])
	print "este script ha creado los siguientes archivos de salida\nGetMacslist_macs_not_exists.txt\nGetMacslist_macs_exists.txt\FilesOutput/GetMacslogs_error.txt"
