...
def InsertElementIntoEquipo(cursor_db, connection, sequence, id_estado, id_tipodispositivo, id_marca, nombre):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la insercion de datos en la tabla equipo (InsertElementIntoEquipo)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
	return insert
...
def InsertIntoUsuariosEquipos(cursor_db, connection_db, id_equipo, id_usuario, sequence):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la insercion de datos en la tabla usuariosequipos (InsertIntoUsuariosEquipos)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetIDInTable(cursor_db, table, field, element, index):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetIDInTable)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def GetEquiposOfUsers(cursor_db, id_tipo_us):
	...
	try:#manejo de errores
		...
		logs.write("No es posible realizar la consulta a la base de datos (GetEquiposOfUsers)")
		sys.exit("ejecucion del script interrumpida, por favor revisar el archivo: TablesRelations_Logs_error.txt")
	logs.close()
...
def InsertEquipos(cursor_db_import, cursor_db_export, connection_db_export, state_device_import, state_device_export, kind_device_import, kind_device_export, trademark_import, trademark_export, tipo_usuario, id_tipo_us):
	...
	print "Insert elements into table equipo and auto completing elements into table usuariosequipos"
